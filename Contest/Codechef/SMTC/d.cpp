...
const int MOD = 1e9+7, N = 3e5 + 2; 
const int SZ = 700;

struct Query {
    int l, r,x ,idx;
    bool operator<(Query const& o){
      if(l/SZ == o.l/SZ){
...
vector<pii> A;
FenwickTree fen(N);
// indexed_set<pii> open,close;

void add(int id){
   // open.insert({A[id].F,id});
   // close.insert({A[id].S+1,id});
   fen.range_add(A[id].F,A[id].S,1);
}

void remove(int id){
   // open.erase({A[id].F,id});
   // close.erase({A[id].S+1,id});
   fen.range_add(A[id].F,A[id].S,-1);
}

int get_answer(int x){
   // int ans = open.order_of_key({x,MOD})-close.order_of_key({x,MOD});
   return fen.sum(x);
   // return ans;
}
