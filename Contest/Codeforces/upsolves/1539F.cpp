...
int32_t main(){
   ...
   for (int i = 0; i < n; ++i) {
      for (int idx : g[i]) {
         int mn = stMin.query(0, i);
         int mx = stMax.query(i, n - 1);
         debug(idx, mn, mx);
         ans[idx] = max(ans[i], (mx - mn + 1) / 2);
      }
      for (int idx : g[i]) {
         stMax.update(idx, n - 1, -2);
         stMin.update(idx, n - 1, -2);
      }
      for (int idx : g[i]) {
         int mx = stMax.query(0, i);
         int mn = stMin.query(i, n - 1);
         debug(idx, mn, mx);
         ans[i] = max(ans[i], (mx - mx - 1) / 2);
      }
   }
   for (int i : ans) {
      cout << i << ' ';
